...
export class DealView {
    ...
    createElement(tag: string, className?: string | string[]) {
        ...
        // Array
        if (typeof className === 'object') className.forEach((item) => {
    ...
    createInput(
        { id, type, placeholder, name, value, required, autofocus, min, step }: Input = {
            id: 'default',
            type: 'text',
            placeholder: 'default',
            name: 'default',
        ...
        const input = this.createElement('input', 'form-control') as HTMLInputElement;
    ...
    generateFormGroup(input: HTMLInputElement) {
    ...
    generateTableListHeader(table: HTMLElement) {
        ...
        const trLegendsItems: string[] = ['# ID', 'DATE', 'QUANTITY', 'VALUE', 'ACTIONS']
    ...
    displayDeals(deals: Deal[]) {
        ...
        // Create nodes
        deals.forEach(deal => {
            const tr = this.createElement('tr') as HTMLTableRowElement;

            const tdDealValues: string[] = [
                deal.id,
    ...
    bindDeleteDeal(handler: Function, deals: Deal[]) {
